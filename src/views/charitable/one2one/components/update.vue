<template>
    <div>
        <Modal v-model="show" title="更新一对一捐款"
            ...
            <Form ref="modalForm" :model="data" :rules="ruls"
                ...
                <FormItem label="省市" prop="donationBrief">
                    <Cascader :data="citysData" trigger="hover" @on-change="citysHandler"
                    ></Cascader>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async getUserInfo() {
                try {
                    let res = await post('/charitable/one2one/get/id/{donationId}', null, {
                        donationId: this.donationId
                    });
                    this.data = res.data;
                    this.data.one2oneType = this.data.one2oneType.toString();
                } catch (error) {
                    this.$throw(error)
                }
            },
            ...
            async update(data) {
                this.loading = true;
                try {
                    let res = await post('/charitable/one2one/update/{donationId}', data, {
                        donationId: this.data.donationId
                    })
                    this.$Message.success("募捐人 " + data.name + " 更新成功");
                    this.cancel(true)
                } catch (error) {
                    this.$throw(error)
                }
